...
menu_layout = [['Arquivos', ['Novo (Ctrl+N)', 'Abrir (Ctrl+O)', 'Salvar (Ctrl+S)', 'Salvar como', '---', 'Sair']],
              ['Ferramentas', ['Word Count']],
              ['Ajuda', ['Sobre']]]
...
window = sg.Window('Editor de texto', layout=layout, margins=(0, 0), resizable=True, return_keyboard_events=True, finalize=True)
window.maximize()
...
def word_count():
    ...
    sg.popup_no_wait('Palavras contadas: {:,d}'.format(word_count))

def about_me():
    '''A short, pithy quote'''
    sg.popup_no_wait('Site: git.elio.rf.gd - Github: elioo08')
